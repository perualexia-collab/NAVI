...
**Phase C** validée avec de vraies données Expérience (voir [`docs/reference/phase-c-real-connection-notes.md`](docs/reference/phase-c-real-connection-notes.md)). **Phase D — scan multi-hôtels**, livrée (D1 infra Redis/BullMQ, D3 robustesse, D2 progression temps réel — voir [`docs/reference/phase-d-notes.md`](docs/reference/phase-d-notes.md)). **Phase E — moteur métier**, terminée et validée avec de vraies données Expérience (E1, E2, E3-P11, E3-P10) — voir [`docs/reference/phase-e-notes.md`](docs/reference/phase-e-notes.md). **Phase F — Audiences**, F1 à F5 validées avec de vraies données Expérience (création de liste, suivi d'action, historique des scans, estimation de temps, dashboard réel). **Phase F6 — suppression réelle (hôtels/utilisateurs) + CRM Health réel**, construite (backend + frontend, typecheck/build OK), pas encore testée — voir les sections Phase F du même document. **Phase G — hôtels & scans par compte NAVI** : G1 (`Hotel.ownerId`) puis **G2** (retour réel) — les hôtels redeviennent un catalogue partagé entre tous les comptes, seuls les scans restent propres à chaque compte (`Scan.requestedById` ; un admin voit tout) — voir [`docs/reference/phase-g-notes.md`](docs/reference/phase-g-notes.md). **Phase H — Ask NAVI (démarrée en avance)**, construite et **validée de bout en bout avec de vraies données** : question → routeur d'intention (H3) → Context Builder (H2) → LLM Service Groq/Qwen (H1) → réponse réelle affichée dans l'UI (H4), avec mémoire conversationnelle et historique persisté en base (H5-H7). Limitation connue : quota gratuit Groq serré (30 req/min, 8000 tokens combinés prompt+réponse/min — modèle passé à `qwen/qwen3.8-27b` le 2026-09-18 après un retrait silencieux de `qwen/qwen3.6-27b` par Groq) — voir [`docs/reference/phase-h-notes.md`](docs/reference/phase-h-notes.md). **Phase I1 — étoiles/chambres/emplacement + comparaison d'hôtels dans CRM Health**, construite et **validée avec une vraie session Expérience** (récupération étoiles/chambres/ville via Administration > Établissement, dérivation d'arrondissement, comparaison de scores CRM entre hôtels) — voir [`docs/reference/phase-i-notes.md`](docs/reference/phase-i-notes.md).
...
2. Renseigner dans `backend/.env` (jamais committé — voir `.gitignore`) :
   ...
   `https://api.groq.com/openai/v1` / `qwen/qwen3.8-27b`).
3. Dans le terminal du Codespace :
   ```bash
