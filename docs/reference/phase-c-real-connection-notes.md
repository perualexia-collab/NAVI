...
| 2026-09-15 (post-clôture, cf. ci-dessous) | Statistiques Marketing (`scrapeMarketingKPIs`) | Sur un hôtel réel, CA Automations affiché = CA Campagnes ponctuelles (ex. 25 887,81 € dupliqué dans les deux), alors qu'Expérience affiche bien 0 € pour les automations — réservations (0 vs 36) correctement lues, seul le CA était faux. | `readMarketingMetric()` exige un motif "montant à 2 décimales + €" (`\d[\d\s]*[,.]\d{2}\s*€`) pour reconnaître la carte CA au niveau courant ; un montant nul affiché par Expérience comme "0" nu (sans "€" ni décimales) ne matchait jamais ce motif, donc la boucle remontait les parents jusqu'à retomber par erreur sur le CA de la carte Campagnes voisine dans un ancêtre commun. Le comptage "Nombre de réservations" n'a pas ce problème (son motif accepte n'importe quel chiffre, y compris seul). | Ajout d'un cas explicite reconnaissant un "0" nu juste après le libellé "Chiffre d'affaires" (avec ou sans "€"/décimales), testé sur le texte immédiatement après le libellé plutôt que sur tout le texte restant — pour ne pas être bloqué par un autre libellé accolé dans la même carte. Non revalidé avec un nouveau scan réel au moment de cette note (pas d'accès à une session Expérience live depuis cet environnement) — à confirmer sur l'hôtel concerné. |
